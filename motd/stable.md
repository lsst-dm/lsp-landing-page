...
Note that the most recent weekly (`w_2019_23`) has a slight annoyance that the terminal upon spawning may appear blank.  To fix this, change the terminal theme to `light` or `dark` via `Settings` --> `Terminal Theme` --> `[Light|Dark]`.
